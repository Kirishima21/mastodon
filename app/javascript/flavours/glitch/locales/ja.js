...
const messages = {
  '':'',
  'account.endorse':'このアカウントをおすすめする',
  'account.posts_with_replies':'投稿と返信',
  'account.unendorse':'このアカウントをおすすめから外す',
  'confirmation_modal.do_not_ask_again': '次からこのメッセージを表示しない',
  'getting_started.open_source_notice': 'Glitchsocは{Mastodon}によるフリーなオープンソースソフトウェアです。誰でもGitHub（{github}）から開発に參加したり、問題を報告したりできます。アスタルテの開発も同様に（{astarte}）で進んでいます。バグの報告はこちらからも受け付けています。',
  'keyboard_shortcuts.hotkey': 'キーボードショートカット',
  'layout.auto': '自動',
  ...
  'settings.general': '一般',
  'settings.hicolor_privacy_icons':'公開範囲のアイコンを表示する',
  'settings.hicolor_privacy_icons.hint':'投稿の詳細ページに公開範囲のアイコンがカラーで表示されます',
  ...
  'settings.media_letterbox_hint':'投稿された画像をスケーリングやメディアの引き伸ばしを行わず、イメージコンテナのサイズの調整と余白の塗りつぶしをして表示する',
  'settings.media_fullwidth': '全幅メディアプレビュー',
  'settings.media_reveal_behind_cw':'コンテンツワーニングを許可なしで全て表示します。',
  'settings.navbar_under': 'ナビを画面下部に移動させる(モバイル レイアウトのみ)',
  'settings.notifications.favicon_badge': '未読件数を表示する',
  'settings.notifications.favicon_badge.hint':'通知のアイコンに未読の通知があるとき件数が表示されます。(モバイル レイアウト)',
  'settings.notifications_opts': '通知の設定',
  'settings.notifications.tab_badge': '未読の通知があるとき、通知アイコンにマークを表示する',
  'settings.notifications.tab_badge.hint':'通知カラムが開いていない時に、未読通知のパッチを通知カラムの列に表示する',
  'settings.preferences': 'ユーザー設定',
  'settings.preselect_on_reply':'前もってリプライ先のユーザー名を指定する',
  'settings.preselect_on_reply_hint':'複数のユーザーとリプライをするときに、最初のユーザーを指定するようにする',
